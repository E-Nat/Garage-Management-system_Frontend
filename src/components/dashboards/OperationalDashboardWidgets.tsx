...
export type DashboardDateFilterPreset =
  | 'today'
  | 'week'
  | 'month'
  | 'custom';
...
export const OperationalDashboardWidgets: React.FC = () => {
  ...
  const [dateFilter, setDateFilter] =
    useState<DashboardDateFilterPreset>('today');
  ...
  const getActiveDateRange = () => {
    ...
    if (dateFilter === 'week') {
      ...
      return {
        ...
        label: `This Week (${startStr} to ${endStr})`,
      };
    }
    ...
    if (dateFilter === 'month') {
      ...
      return {
        ...
        label: `This Month (${startStr} to ${endStr})`,
      };
    }
  ...
  const isDateInRange = (dateStr?: string): boolean => {
    ...
    if (activeRange.start && extracted < activeRange.start) {
      return false;
    }

    if (activeRange.end && extracted > activeRange.end) {
      return false;
    }
  ...
  const filteredCustomers = customers.filter((customer) =>
    isDateInRange(customer.createdAt)
  );
  ...
  const filteredVehicles = vehicles.filter((vehicle) =>
    isDateInRange(vehicle.createdAt)
  );
  ...
  const filteredRepairJobs = repairJobs.filter((job) => {
    ...
    return history.length > 0
      ...
      : isDateInRange(
          job.entryDate || job.serviceDate || job.receivedDate
        );
  });

  const enteredStatusCount = (statuses: string[]): number =>
    repairJobs.filter((job) => {
      ...
      if (history.length > 0) {
        return history.some(
          (entry) =>
            statuses.includes(entry.toStatus) &&
            isDateInRange(entry.timestamp)
        );
      }
  ...
  const pendingInspectionCount = enteredStatusCount([
    'pending_inspection',
    'checked_in',
    'diagnosing',
  ]);

  const waitingApprovalCount = enteredStatusCount([
    'waiting_approval',
    'waiting_parts',
  ]);
  ...
  const completedCount = enteredStatusCount([
    'completed',
    'ready',
  ]);
  ...
  const paidInvoicesInRange = invoices.filter(
    (invoice) =>
      ...
      isDateInRange(
        invoice.paidAt ||
          invoice.issuedAt ||
          (invoice as unknown as { createdAt?: string }).createdAt
      )
  );

  const calculatedRevenue = paidInvoicesInRange.reduce(
  ...
  const handleStatusCardClick = (statusKey: string): void => {
    sessionStorage.setItem(
      'repair_job_status_filter',
      statusKey
    );
  ...
  const handlePresetSelect = (
    preset: DashboardDateFilterPreset
  ): void => {
    ...
    <div className="space-y-6 text-slate-900 font-sans">
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {(
            ['today', 'week', 'month', 'custom'] as DashboardDateFilterPreset[]
          ).map((preset) => {
            ...
            return (
              ...
              >
                {preset === 'custom' && (
                  <Filter className="w-3 h-3 inline mr-1 text-slate-400" />
                )}
      ...
      {dateFilter === 'custom' && (
        ...
        >
          <div className="flex items-center gap-2">
            <label
              htmlFor="custom-start-date-input"
        ...
        >
          <div>
            <div className="flex items-center gap-2">
              ...
              {/* FIXED:
                  Removed dateFilter !== 'all'
                  because 'all' is not part of the type.
              */}
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <button
            ...
            type="button"
            onClick={() =>
              handleStatusCardClick('pending_inspection')
            }
          ...
          <button
            ...
            type="button"
            onClick={() =>
              handleStatusCardClick('waiting_approval')
            }
            className="bg-white hover:bg-slate-50/70 p-4 rounded-xl border border-slate-100 shadow-sm transition-colors text-left flex flex-col justify-between cursor-pointer"
          >
            <div className="flex items-center justify-between text-slate-400 mb-2">
          ...
          <button
            ...
            type="button"
            onClick={() =>
              handleStatusCardClick('in_progress')
            }
            className="bg-white hover:bg-slate-50/70 p-4 rounded-xl border border-slate-100 shadow-sm transition-colors text-left flex flex-col justify-between cursor-pointer"
          >
            <div className="flex items-center justify-between text-slate-400 mb-2">
          ...
          <button
            ...
            type="button"
            onClick={() =>
              handleStatusCardClick('completed')
            }
            className="bg-white hover:bg-slate-50/70 p-4 rounded-xl border border-slate-100 shadow-sm transition-colors text-left flex flex-col justify-between cursor-pointer"
          >
            <div className="flex items-center justify-between text-slate-400 mb-2">
          ...
          <button
            ...
            type="button"
            onClick={() =>
              handleStatusCardClick('delivered')
            }
            className="bg-white hover:bg-slate-50/70 p-4 rounded-xl border border-slate-100 shadow-sm transition-colors text-left flex flex-col justify-between cursor-pointer"
          >
            <div className="flex items-center justify-between text-slate-400 mb-2">
          ...
          <button
            ...
            type="button"
            onClick={() =>
              handleStatusCardClick('declined')
            }
            className="bg-white hover:bg-slate-50/70 p-4 rounded-xl border border-slate-100 shadow-sm transition-colors text-left flex flex-col justify-between cursor-pointer"
          >
            <div className="flex items-center justify-between text-slate-400 mb-2">
